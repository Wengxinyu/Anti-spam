...
def linear_batch_gd(data, theta, learning_rate):
    temp = [0]*58
    for each_data in data:
        h = 0.0
        for j in range(num_attribute + 1):
            h += each_data[j] * theta[j]
        for j in range(num_attribute + 1):
...
def runAndPrint(num_fold, filename):
    ...
    # f = open('Linear_BGD_0.00001.csv', 'w')
    fig = plt.figure()
    for k in range(10):
        ...
        for epoch in range(iterations):
            ...
            linear_batch_gd(train_set, theta, BGD_learning_rate)

            for index_data in range(num_train):
                error += linear_regression(train_set[index_data], theta)
            J = error / (2 * num_train)
