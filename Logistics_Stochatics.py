...
def logistics_stochastic_gd(each_data, theta, learning_rate):

    h_temp = 0.0
    for i in range(num_attribute + 1):
        h_temp += each_data[i] * theta[i]
    h = float(sigmoid(h_temp))
...
def runAndPrint(num_fold, filename):
    ...
    # f = open('Logistics_SGD_0.1.csv', 'w')
    fig = plt.figure()
    for k in range(10):
        ...
        for epoch in range(iterations):
                ...
                for index_data in range(num_train):
                    logistics_stochastic_gd(train_set[index_data], theta, SGD_learning_rate)
